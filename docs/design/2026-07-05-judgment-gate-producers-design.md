...
- **emit**: check_id `gate.intent_fidelity`, `measured.intent_fidelity = {value, source:"claims_backing",
  ...
  measure_submission 이 `--from-evidence` 로 승계하는 입력이다.) 승계된 값은
  `scoring.correctness` 의 assertion `intent_fidelity >= 0.7` 로 phase-09 에서 직접
  게이팅된다 — 0.0(required 미충족)은 하드 FAIL, 0.7·1.0 은 통과. (B4 드레스 리허설
  실측 갭 #1 클로즈, 2026-07-05 — 초판은 도메인 assertion 만 있어 0.0 이 value 0.0 인
  채 PASS 였다.)
