...
## 한 줄 요약
**한 줄 요약 + 점수 시계열 + 자율 결정 이력 + 웹뷰 실행 명령 + (자율 권한 시) PR 생성**. 본 stub 은 형태와 인터페이스만 정의 — 콘텐츠는 [`../theseus-harness/`](../theseus-harness/) 단일 source of truth.

## 입력 산출물 (frontmatter 검증)

- `모든 페이즈 산출물 + sprint 결과 + multiverse 결과`
...
```bash
/theseus-handoff --from <input_dir>
```
...
```bash
# 이미 의도 산출물 있음 → 본 스킬부터
/theseus-handoff --from .ShipofTheseus/<프로젝트>/
```
