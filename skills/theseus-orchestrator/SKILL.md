...
## HARD-RULE — 본 스킬을 호출한 직후 반드시 (sprint-03-a, v0.8.0)
...
본 HARD-RULE 은 livetest 시나리오 #1 (G2 url-shortener) 의 fail 분석에서 도출. sub-claude 가 본 스킬을 호출하지 *않고* 직접 Go 구현 + retroactive `_tools/build_artifacts.py` 작성으로 우회. 페이즈 산출물 0 → verify 5 체크 중 4 fail. 본 룰이 정정 (sprint-03-a). 상세 finding: `.tests/results/01-url-shortener-g2/finding.md`.
