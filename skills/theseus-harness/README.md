...
## 한 줄 요약
**한 요구를 처음 의도한 타이틀로 끝까지 부를 자격을 만드는 재귀 멀티 에이전트 코딩 하네스 (Claude Code 스킬, v0.2.0).** 메인 진입점은 [`SKILL.md`](SKILL.md). 명명→의도+마인드맵→문서화→교차 이해→사용자 질의+유즈케이스+스택 점검→비평→계획+시퀀스→재계획→구현+빌드 스크립+TOML→5종 게이트(DIP 우선)→무한 스프린트(임계 0.999)→회귀 바이섹트→bun 웹뷰→핸드오프.
