...
d- [`conventions/`](conventions/) — 47 컨벤션 모듈 (각 파일 명시):
  ...
  d-36 [`conventions/domain-pack.md`](conventions/domain-pack.md) — 도메인 layer 통합: §2 model completeness (built-in 5 차원) + §3 research stacking (framework) + §4 failure patterns (framework). 본 하네스 built-in 어댑터 0 (sprint-19+, 벤치 어뷰징 회피) — sprint-37 PR-AG 통합 (v0.9.13 + v0.9.18 + v0.9.22)
  ...
  d-53 (제거됨, sprint-19+) — built-in 도메인 어댑터 0. 사용자 per-project 제공 패턴은 `domain-pack.md` §3 (sprint-37 PR-AG 통합) 프레임워크 참조 (벤치 어뷰징 회피)
  ...
  d-78 [`conventions/intent-refresh.md`](conventions/intent-refresh.md) — 1차 (phase 04 종료) + 2차 (phase 05 종료) refresh 단일 컨벤션. 4 framing universe (domain/constraint/risk/outcome) + 01-additional + 04/05-refreshed cascade (sprint-17 by + sprint-19 ci, sprint-37 PR-AA 통합)
