...
- `score.py` — 6 차원 weighted 보고(설계 B2 §2.3 — 절대 임계 게이트 아님), DIP hard cap 0.6
- `fingerprint.py` — frontmatter 핑거프린트 + 체인 무결성
- `self_lint.py` — 본 저장소 자기 검증 (`--score` 보고 모드, 판정은 `all_ok`)
...
| **Grade 4** Complex (default) | FE+BE / 새 도메인 / SOLID 리팩터 | 15 페이즈 풀 / stop_policy(manifest) |
| **Grade 5** Mission Critical | 결제 / 금융 / 안전 | 15 풀 + 빡빡 / stop_policy plateau_window 3 |
...
self_lint `--score` 는 보고 전용(설계 B2 §2.3) — 본 저장소가 자기 강제 룰을 100% 통과하는지는 `all_ok` boolean 이 판정.
