...
version: 0.9.20
description: 재귀 멀티 에이전트 코딩 하네스 — 15 페이즈 / 62 컨벤션 / 18 에이전트. AIDE 멀티버스 (G3=5/G4=7/G5=9 폭 default) + 발현 검증 + Layer 3 결과물 허들 + grade_assess v2 + intent §k 9 sub + cycle coherence + domain failure patterns + decision-support framing + v0.9.19 sprint-13 (mindmap A default / per-module diagram / width bump / trinity 3 axis × ≥2) + v0.9.20 sprint-14 (grader-in-sprint dual-objective AND / contested decisions multiverse axis / directional simplification / commentary policy / measurement contract / rubric-driven doc skeleton + targeted gates).
---
