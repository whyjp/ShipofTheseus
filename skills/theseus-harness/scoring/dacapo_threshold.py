...
Exit codes:
    0 — winner ratio ≥ threshold (round N+1 불필요, phase advance 허용)
    1 — winner ratio < threshold (round N+1 강제, phase advance 차단)
...
def evaluate(
    ...
    return {
        ...
        'next_round_required': not passed,
        'reason': (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=f'다카포 임계 (default {DEFAULT_THRESHOLD})',
    )
    parser.add_argument(
        '--output',
        type=Path,
    ...
    if not ns.quiet and verdict_obj['verdict'] == 'fail':
        print(
            f"\n[dacapo_threshold] FAIL: {verdict_obj.get('reason', 'verdict=fail')}",
            file=sys.stderr,
        )
        print(
            f"[dacapo_threshold] orchestrator 의무 — round N+1 자동 진행 (HARD-RULE 9.qq).",
            file=sys.stderr,
        )
