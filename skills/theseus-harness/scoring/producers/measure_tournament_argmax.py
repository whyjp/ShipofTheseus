...
디스크에서 파싱해 emit:
  ...
  - winner_subscores_count       : frontmatter winner_sub_scores 파싱된 차원 수(정보용)
  - winner_subscore_hull_delta   : winner_score 가 자기 sub-scores [min,max] 밖으로 벗어난 거리
                                    (0=범위 안/부재; convex hull 하한 — 가중치 재계산 없이 위조 탐지)
...
def _parse_winner_sub_scores(text: str) -> list[float | None]:
    """frontmatter 의 nested `winner_sub_scores:` 블록 → 값 리스트(미파싱 항목은 None).

    _parse_frontmatter 는 최상위만 파싱(들여쓴 줄 skip)하므로 nested 블록은 여기서 따로 읽는다.
    최상위 `winner_sub_scores:`(빈 값) 줄을 찾고 이후 들여쓴 줄들을 dedent 까지 소비. 각 줄은
    인라인 주석(#) 제거 후 `key: value` 로 파싱(스키마 예시가 `0.72  # weakest` 형태를 씀)."""
    m = _FRONTMATTER_RE.match(text)
    if not m:
        return []
    ...
    for line in m.group(1).split("\n"):
        if not in_block:
            if line[:1] not in (" ", "\t") and re.match(r"^winner_sub_scores:\s*(#.*)?$", line.strip()):
                in_block = True
            continue
        if line[:1] not in (" ", "\t"):
            break  # dedent → 블록 종료
        ...
        if not s or s.startswith("#") or ":" not in s:
            continue
        _, _, val = s.partition(":")
        subs.append(_parse_num(val.split("#", 1)[0]))  # 인라인 주석 제거 후 파싱
    return subs
...
def _build_report(run_root: Path, text: str, fm: dict[str, str]) -> dict[str, Any]:
    ...
    # winner sub-score convex hull: 어떤 가중 평균도 [min,max] 밖 값을 못 낸다. winner_score 가
    # 자기 sub-scores 범위 밖이면 위조/Σ-단순합산(weight-free 하한 검증 — 가중치 재계산 없음).
    # 부재(count 0)/미파싱(subset None)/winner_score 부재 시 0.0(관용 — winner_row_delta 가
    # 서술 일관성을 별도 커버; sub-scores 는 REQUIRED_TOURNAMENT 아님이라 기존 run false-FAIL 방지).
    subs_raw = _parse_winner_sub_scores(text)
    winner_subscores_count = sum(1 for v in subs_raw if v is not None)
    if winner_subscores_count == 0 or any(v is None for v in subs_raw) or winner_score_fm is None:
        winner_subscore_hull_delta = 0.0
    else:
        lo, hi = min(subs_raw), max(subs_raw)
        winner_subscore_hull_delta = round(max(0.0, lo - winner_score_fm, winner_score_fm - hi), 6)
    ...
    return {
        ...
        "winner_subscores_count": winner_subscores_count,
        "winner_subscore_hull_delta": winner_subscore_hull_delta,
    }
...
_MEASURED_KEYS = (
    ...
    "winner_subscores_count",
    "winner_subscore_hull_delta",
)
