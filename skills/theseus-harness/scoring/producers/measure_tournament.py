...
비율(다른 artifact·다른 차원)을 value 로 emit 한다 — 구 ≥0.999 게이트 assertion 은 도달
불가 임계 perverse incentive 라 삭제됐다(설계 B2 §2.3; 정지 판정 권위는 manifest
stop_policy). 여기 winner_ratio 는 shadow grader 패널 평균이며 애초부터 gating assertion
대상이 아니라 value 일 뿐 — 어느 쪽도 절대 임계로 게이트하지 않으므로 중복/충돌 없다.
