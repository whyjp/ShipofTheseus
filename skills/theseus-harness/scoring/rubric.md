...
## 한 줄 요약
**총점 ≥ 0.999 일 때 하네스가 종료된다.** 자율적으로 최대 결과를 지향하기 위한 빡빡한 임계. 점수는 6 차원의 가중 평균이며, 각 차원은 `[0, 1]`. **DIP 위반 = `scoring.solid` 차원 게이트 FAIL + 총점 hard cap 0.6 (이중 방어).** 1 차 경로(커널)에서 DIP 위반은 solid 차원을 만점이 아니라 *게이트 FAIL* 로 만들고, 집계(`aggregate_scores`)가 총점을 다시 0.6 으로 눌러 어떤 다른 차원이 만점이어도 임계 미달을 강제한다. 가중치 합은 1.0. 임계는 `score.py --threshold` 로 튜닝 가능하지만, 0.999 미만 임계 사용은 사용자 명시 동의 필요.
...
| **0.99999** (자기 평가 임계, BOOTSTRAP.md) | self_lint 35/35 pass + pytest 99/99 pass + sample_score 1.0 의 가중 평균 | 본 저장소의 *마크다운 텍스트 인덱스 정합성* 측정값. LLM 행동 정확도가 아님. |
