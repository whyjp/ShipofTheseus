...
ⓐ 비평가(페이즈 05) 가 "대안 A·B 의 트레이드오프가 비슷, 사용자 결정도 어려움" 으로 보고.
ⓑ 설계자(페이즈 06) 가 "두 모듈 분할안의 장단점이 길항" 으로 보고.
ⓒ 구현자(페이즈 08) 가 "두 구현 방식이 모두 SOLID/테스트 통과 가능, 미감 차이만 있음" 으로 보고.
ⓓ 플랜 재이해(페이즈 07) 의 콜드 리딩이 두 가지 다른 해석을 동시에 산출.
ⓔ 같은 페이즈를 두 번 호출했는데 LLM 이 의미상 다른 결과를 출력 (비결정성 직접 노출).
...
ⓐ 페이즈 06 (계획) 경쟁이라면 — `agents/plan-reviewer.md` 의 콜드 리딩 4 답을 *후보별 별도 fresh 에이전트* 로 받아 비교. 더 잘 인코딩된 안이 우승.
ⓑ 페이즈 08 (구현) 경쟁이라면 — `quality-gate.md` 5 게이트 + `score.py` rubric 으로 후보별 점수 산출. **DIP 위반은 단독 hard cap 0.6** ([`../scoring/rubric.md`](../scoring/rubric.md)) 가 후보 탈락 기준.
ⓒ 다른 페이즈는 그 페이즈의 자기 성공 기준에 맞춘 *비교 rubric* 을 `meta.md` 에 미리 정의.
...
ⓐ **노이즈 분리** — 같은 입력에 LLM 이 표면 차이만 다르게 출력하면 두 후보의 점수가 거의 같다 (Δ < 0.02). 머지로 평균화.
ⓑ **의미 분기 발견** — 의미상 다른 두 해석을 LLM 이 동시 산출하면 점수가 의미 있게 갈린다 (Δ ≥ 0.05). 우승자 채택.
...
ⓑ 점수 차 ≥ 0.05 의 우승자 선택도 자동 (게이트가 객관적이니).
ⓒ 단, 비평가가 *전략적* 의미가 있다고 표시한 경쟁 (예: 인증 모드 선택 같은 비즈니스 함의) 은 머지 직후 사용자에게 [`interview.md`](interview.md) 컨벤션으로 보고 + 4개 보기 중 1 선택:

```
질의: 인증 모드 경쟁 결과를 적용할까요?

후보 a (세션 쿠키, 점수 0.987) 와 b (JWT, 점수 0.972) 가 격렬하게 경쟁했고 a 가 약간 우세했습니다. 머지 안은 a 채택 + b 의 리프레시 흐름만 후속으로 보류.

선택지:
1. 결과 그대로 적용 (a 채택 + b 후속 보류)
2. b 로 강제 변경 (점수와 무관하게 비즈니스 결정)
3. 다시 경쟁 (다른 평가 기준)
4. 정지 — 사람이 직접 비교
```
...
ⓐ **페이즈 06 (계획)** — 가장 큰 가치. 모듈 분할의 애매함을 여기서 해소하면 후속 모든 비용 절감.
ⓑ **페이즈 08 (구현)** — 도메인 코어 같은 복잡 모듈에 한정. 단순 어댑터까지 경쟁시키면 비용 폭발.
ⓒ **페이즈 11 (회귀 바이섹트)** — `revert` vs `re-architect` 권고가 길항할 때.
ⓓ 다른 페이즈는 비용·이득 균형이 안 맞아 보통 적용 안 함.

## 안티 패턴

ⓐ 모든 페이즈에 무조건 경쟁 — 비용 폭발, 가치 없음.
ⓑ 후보 5개 이상 — 비교가 노이즈에 묻힘. 최대 3.
ⓒ 후보 디렉터리끼리 산출물 공유 — 격리 위반, 점수 부정.
ⓓ 점수 차 < 0.02 인데 우승자 강제 선택 — 노이즈를 의미로 오해. 머지 권고 무시 금지.

## 비용 가드

ⓐ 한 페이즈에 경쟁 1회까지. 같은 페이즈에서 두 번 경쟁이 트리거되면 의도/계획 자체가 모호 → 페이즈 04 사용자 질의로 회귀.
ⓑ 경쟁 중인 페이즈는 [`timing.md`](timing.md) 사용자 보고에 "경쟁 N 후보 진행 중" 명시 — 사용자가 비용·시간 사실 인지.
ⓒ 동시 디스패치 후보 수는 [`models.md`](models.md) 의 모델 비용을 고려해 결정 — Opus 후보가 3개면 합리적, Sonnet 은 5개도 가능.
