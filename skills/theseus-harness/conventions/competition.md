...
## 자동 머지 알고리즘 (sprint-05-b)

폭 확장 (G3 폭 3 / G4 폭 4 / G5 폭 6) 으로 후보 수 증가 → *자동 머지 알고리즘* 강화 필수. 단일 우승자 vs 머지 결정을 결정론적으로 자율 처리.

### head-to-head 점수 비교 — 차원별 sub-score

각 후보 universe 의 점수를 *차원별로 분해* 해 *어느 universe 가 어느 차원에서 강한지* 식별. 결과 = 단일 합계가 아닌 차원별 매트릭스.

| 차원 (예시 — 외부 도메인 매핑) | 가중 | 측정 출처 |
|----|----|----|
| Conceptual (의도/모델 정합) | 0.20 | doc-reviewer score |
| Data fidelity (데이터 충실) | 0.15 | quality-gate |
| Correctness (단위 + 통합 테스트) | 0.20 | sprint pytest pass rate |
| Experimental design | 0.15 | tester rep count + CI 품질 |
| Results interpretation (산출 narrative) | 0.15 | doc-reviewer |
| Code quality (DIP + lint) | 0.10 | quality-gate + ruff |
| Traceability | 0.05 | event log / fingerprint chain |

### 자동 resolve 규칙 (head-to-head)

a- **단일 우승자** : 한 universe 가 *모든 차원* 에서 ≥80% 만점 + 다른 universe 대비 합계 차 ≥0.05 → 우승자 단독 채택.
b- **차원별 머지** : 각 차원의 *최강 universe* 가 다르면 → 차원별로 다른 universe 의 컴포넌트 추출 머지 (예: U1 의 Resource topology + U2 의 Dispatcher + U3 의 EventLogger).
c- **재경쟁** : 합계 차 < 0.02 + 차원별 강점 분리 안 되면 → planner 에 "더 명확한 분기 시드" 요청 + 1회 재실행.
d- **타임아웃** : wall-clock budget 초과 시 *현 시점 최고 합계 universe* 단독 채택 + handoff/14 의 `intervention.category` 에 "auto-resolve-timeout" 기록.

### sprint-05-a 베이스라인 케이스 적용 예

simulation-bench 베이스라인 (sprint-05-a) 의 U1 (process-first / per-direction) vs U2 (hybrid centralized / shared bidirectional) → 본 알고리즘 적용 시 :
- Conceptual : U2 강 (metadata "same physical constraint" 정확 해석)
- Sim correctness : U1 강 (idiomatic SimPy)
- Traceability : U2 강 (central EventLogger)
→ **차원별 머지 채택** : U2 Resource + U1 Dispatcher + U2 EventLogger (실제 sprint-05-a 머지 결정과 일치)

자동 알고리즘이 사람 critic 의 결정과 같은 답에 도달 가능 — 본 sprint-05-b 의 핵심 가치.
