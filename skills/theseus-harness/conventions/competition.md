...
ⓐ 점수 차 < 0.02 의 머지 결정 — 자동 (Q-D2 답 무관).
ⓑ 점수 차 ≥ 0.05 의 우승자 선택 — 자동 (게이트가 객관적).
ⓒ 비평가가 *전략적* 의미 있다 표시한 경쟁 (예: 인증 모드) — [`autonomy.md`](autonomy.md) **Q-D2 사전 위임 답** 자동 적용:
  ⓒ-1 Q-D2 답 = `1` (모든 resolve 자동) → 점수 우위 자동 채택.
  ⓒ-2 Q-D2 답 = `2` (비즈니스 함의도 자동) → 동일하게 점수 우위 자동.
  ⓒ-3 Q-D2 답 = `3` (비즈니스 함의는 정지) → 본 스킬 의도 위반, 페이즈 04 에서 비권장 표시되었으나 사용자가 선택했다면 정지.
