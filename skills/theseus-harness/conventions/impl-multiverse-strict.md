...
g- **mandatory rerun 자체는 advisory (sprint-28 j, 설계 B2 §2.2-3 재정정)** — 구판은 "budget 충분 시 임계 도달까지 무한 회귀"를 강제했으나, phase 08 의 종료 권위는 이제 manifest `stop_policy`(gate AND no_regression AND (plateau OR budget≥95%)) 다. "No further sprints required" 류 확신 표현은 [`conservative-margin-judging.md`](conservative-margin-judging.md) 의 근거 없는 확신 sentinel 검토 대상이나, 실측 plateau 를 정직 보고하는 것은 위반이 아니다.
