...
d- **** 모듈 ≥ 4 인데 *단일 통합 시퀀스만* 출력 — 가독성 0 + cold review 분할 불가능. [`per-module-diagram-fan-out.md`](per-module-diagram-fan-out.md) (bb) 의 trigger 조건 (모듈 ≥ 4 OR consumer-producer 페어 ≥ 6) 시 per-module 다이어그램 ≥ 모듈 수 의무 (C-PMDF, 미등록).
...
- universe candidate (G3+) 도 동일 — [`aide-tree.md`](aide-tree.md) §3 (sprint-37 PR-AB 통합) 정합.
