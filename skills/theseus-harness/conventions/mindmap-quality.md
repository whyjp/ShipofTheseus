...
각 페이즈 산출물 frontmatter 에 `mindmap_nodes_referenced: [...]` 강제 — 본 페이즈가 마인드맵의 *어느 노드* 를 다뤘는지 명시. **C-MM** 이 본 frontmatter 키 누락 자동 fail.
...
**C-MQG-EVOLVE** = 페이즈 진행 중 mindmap_revision 가 *증가* 했는지 검증. 페이즈 02-09 동안 revision 0 = 가드닝 누락.
...
- C-MQG-FORMAT: ```mermaid mindmap``` 패턴 매칭 ≥ 1
```
...
**C-MQG-WIDTH** = 4 axis 모두 ≥ 3 sub-node.
...
**C-MQG-DEPTH** = 4 axis 중 ≥ 2 axis 가 sub-sub-node ≥ 1.

### 3.4 풍성도 임계 — 총 노드 ≥ 15 (B) / ≥ 25 (A)

**C-MQG-RICHNESS** = mindmap 의 leaf + intermediate 노드 합 ≥ 15 (B 임계) — A 등급 임계는 §4 참조.
...
**C-MRD-A-DEFAULT** = `mindmap_quality_grade` frontmatter PASS 조건: 'A' (G4 default) 또는 'B' (with lesson). fail 조건: 'C' (G4+ 시) 또는 'D'.
...
b- frontmatter `mindmap_nodes_referenced` 누락 — C-MM 위반.
