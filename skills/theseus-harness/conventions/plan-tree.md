...
| Grade | 폭 default (sprint-13 v0.9.19) | 옵션 default (사용자 명시 ack) | 깊이 cap | 분기 시드 (필수) | 분기 시드 (옵션) |
| ----- | :---------------: | :---------------: | :------: | ---------------- | ---------------- |
| **G1** Trivial | 1 (트리 비활성) | n/a | — | — | — |
| **G2** Simple | 2 | n/a | 1 | domain-first | minimal |
| **G3** Standard | **5** (← 2/3) | 10 | 1 | 5 axis 카탈로그 상위 5 (process/data/sync/centralized/dynamic/push) | — |
| **G4** Complex | **7** (← 3/4) | 12 | 1 (옵션 2) | 5 시드 + 2 axis (FE+BE × stance) | strict-layering |
| **G5** Critical | **9** (← 5/6) | 16 | 2 (강제) | 5 시드 모두 + 4 axis 추가 | depth 2 자식 분기 |

**v0.9.19 sprint-13 폭 격상** ([`multiverse-width-default-bump.md`](multiverse-width-default-bump.md) bc) — sprint-05-b 후 cold session 4 universe plateau, *5 universe 이상 head-to-head 분기* 의 의미 분기 다양성 미발현. spread default 격상 + budget profile cap 동기 갱신 ([`resources.md`](resources.md) universe parallel cap G3=10/G4=12/G5=16) + budget tight 시 fallback 폭 + `fallback_reason` frontmatter 의무 ([`budget-aware-fallback.md`](budget-aware-fallback.md)). self_lint C-MWDB 가 검증.
...
## 패배 universe 학습 전이 (cross-universe-lesson-distillation 정합)

[`cross-universe-lesson-distillation.md`](cross-universe-lesson-distillation.md) — tournament resolve 시 *우승 universe 만 채택* 으로 끝나지 않고, 패배 universe 의 *핵심 약점 ≥ 1-2 줄* 을 우승 본문에 흡수 의무. forward exploration (본 컨벤션) + backward weakness transfer (cross-universe-lesson) 결합으로 multiverse 의 *모든 학습* 이 다음 페이즈로 전이.
