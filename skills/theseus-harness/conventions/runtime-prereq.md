...
a- 실 secret 의 git 커밋은 **자동 차단** — 자율로 거부 + `.gitignore` 자동 추가 + handoff/13 에 사후 보고. 사용자 ack 호출 없음 (페이즈 04 외 인터럽트 0).
b- `.env` 의 untracked 잔존이 핸드오프 직전 발견되면 본 하네스가 자동 `.gitignore` 추가 + handoff/13 에 명시 (`env_untracked: true`). ack 호출 없음.
...
b- **`.env` 를 git 에 커밋** — 자율 차단 + `.gitignore` 자동 추가 + handoff/13 사후 보고. ack 없음.
...
e- ✅ 페이즈 12 웹뷰의 새 탭 "Runtime" (sprint-02-e #6 출하). `boot_exit` / `healthz` / `secrets_count` / `entry_blocked` / `mode` 라이브 표기. 5 초 간격 `/api/runtime` 폴링.
