...
(B3 — frontmatter 5 조건 자동검증 및 min-loop-attempt 검증은 self_lint 개별 룰에서 삭제됐다. 게이트 실체는 `run_gate.py` + `plan.dacapo_threshold`/`plan.tournament_independence`(커널 값 실측)이며, 위 표는 frontmatter 필드 스키마 자체의 문서 정합만 다룬다.)
...
a- **게이트 SKIP** — orchestrator 가 tournament.md frontmatter 검증 안 하고 phase 07/09 진입 → 본 컨벤션 무력화. `run_gate.py`(`plan.dacapo_threshold`/`plan.tournament_independence`)가 강제.
...
g- ** min loop attempt 우회** — rerun 0 회로 BUDGET_OVERRUN 직행. 최소 1 회 실 rerun 후에만 cap 발동(본 컨벤션 §2 조건 3b 의무).
...
|---|---|---|---|
| step_e_cap_reached | false | 조건 3b min loop attempt (rerun=0 인데 cap claim) | **FAIL** |
