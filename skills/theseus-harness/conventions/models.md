...
ⓐ Opus 호출이 한 페이즈에서 3 회 이상 반복되면 (재실행 루프) 사용자에게 비용 사실 한 줄 보고.
ⓑ Haiku 가 출력 품질 부족으로 fail 한 페이즈는 *한 번만* Sonnet 으로 escalate, 그래도 안 되면 사용자 질의.
ⓒ 사용자가 명시적으로 "전부 Sonnet" / "전부 Opus" 지정 시 그 결정을 우선.

## 비고

ⓐ 본 매핑은 휴리스틱 — 실제 작업의 도메인 복잡도에 따라 implementer 처럼 페이즈 안에서 모델을 선택적으로 끌어올릴 수 있음.
ⓑ 모델 명세 변경 시 본 문서를 source of truth 로 — SKILL.md 의 페이즈 표는 본 문서를 참조.
