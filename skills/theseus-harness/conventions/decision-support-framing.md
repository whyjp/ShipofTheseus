...
예 (DES): "8 트럭 = 현재 crusher service rate 의 sweet spot. 12 트럭 = 동일 throughput 에 queue 3x 증가 — 운영자 부담 + 트럭 wear 비용 동시 증가."
예 (Web API): "RPS 200 = current DB pool 의 sweet spot. RPS 400 = same throughput, p99 latency 5x — UX 회복 불가."

### Trade-off framing
<주어진 결정의 *대안* 들 + 비교 차원>
예: "8 트럭 vs 12 트럭 vs 8 트럭 + faster crusher — 비용/throughput/queue/wear 4 차원 비교 표"

### Opportunity-cost
<선택 시 *포기하는* 것 명시>
예: "12 트럭 채택 = 4 트럭 추가 capex + maintenance overhead 부담. 동일 자본 으로 crusher 업그레이드 가능 (다음 스프린트 후보)."
```
...
b- **opportunity-cost 형식적 ("기회 비용 있음")** — *명시 옵션* 의무 (예: "12 트럭 capex = crusher 업그레이드 capex 와 동급 — 다음 스프린트 후보").
