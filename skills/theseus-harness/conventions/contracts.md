...
a- **부분 호출** — 사용자가 인터뷰만 먼저 돌리고, 결과 파일을 다른 시점·다른 머신에서 던져 플랜 페이즈만 따로 호출하고 싶다.
b- **단계 재진입** — 회귀 후 "스프린트 3 부터 다시" 같은 정확한 진입점.
c- **모듈 분해 가능성** — 향후 페이즈 그룹별로 별도 스킬로 분해할 때, 산출물 계약만 지키면 스킬 간 결합 없이 통신.
d- **출처 보장** — 산출물이 본 하네스가 만든 것인지 확신 (위·변조·잘못된 입력 거부).
...
a- `canonical_body` = 본문 전체에서 frontmatter, timing 헤더, 후행 공백, 행 끝 정규화(`\n` 통일) 후의 텍스트.
b- `skill_version_major` 만 사용해 마이너/패치 변경에는 핑거프린트가 흔들리지 않게.
c- `prev_fingerprint` 가 들어가므로 페이즈 간 체인이 형성 — 한 단계 변조되면 그 이후 모두 무효.
...
3- 가장 늦은 valid 페이즈 N 식별. 페이즈 N+1 부터 시작.
4- 일관성 깨진 산출물이 있으면 사용자에게 객관식 질의 ([`interview.md`](interview.md) 컨벤션):
...
a- "**인터뷰만**" — 페이즈 00–05 만 실행, `.ShipofTheseus/<프로젝트>/` 의 `naming/`, `intent/` 만 산출. 이후 종료.
b- "**플랜만**" — 위 산출물을 입력으로, 페이즈 06–07 만 실행 후 `plan/` 산출.
c- "**구현부터 끝까지**" — 위 + plan 산출물을 입력으로, 페이즈 08–13 까지.

CLI 슬래시 명령에 페이즈 범위를 지정할 수 있게 하는 것도 후속 PR 후보 — `/theseus-harness --from 06 --to 07`.

## frontmatter 가 산출물에 박히는 시점

a- 각 페이즈의 서브에이전트가 산출물을 작성할 때 직접 `fingerprint.py compute` 호출해 결과를 frontmatter 에 기입.
b- 또는 산출물을 본문만 작성한 뒤 페이즈 종료 시점에 지휘자가 helper 로 일괄 부착.
c- 어느 방식이든, 지휘자가 페이즈 *완료* 로 표시하기 전에 frontmatter 가 부착되어야 함.
...
a- MAJOR — 산출물 형식 / 페이즈 인덱스 / 컨벤션 호환성 깨는 변경.
b- MINOR — 페이즈 추가, 컨벤션 추가, 후방 호환 변경.
c- PATCH — 문서 오타, 에이전트 프롬프트 미세 조정.
