...
## universe N 병렬 budget profile (활성 폭 = manifest 단일 권위, 격상 §8 동결)

플랜-트리 폭 = `pipeline.manifest.json` `multiverse_widths`(활성 G3=3/G4=4/G5=6) → 동시 N planner sub-agent + N implementer sub-agent. 격상 폭(`frozen_widths` G3=5/G4=7/G5=9, 옵션 default G3=10/G4=12/G5=16)은 [`multiverse-width-default-bump.md`](multiverse-width-default-bump.md) 동결(advisory) — 사용자 명시 ack 시 가능. 본 절은 *병렬 universe 메모리 가드* 와 *wall-clock budget* 을 정의해 OS OOM / 타임아웃 자율 회피(가드 로직은 활성·격상 폭 양쪽에 적용 가능한 채로 존치).

### 병렬 universe 메모리 가드

| Grade | 동시 universe 수 (활성 폭) | universe parallel cap (격상 시 옵션) | 메모리 가드 | 회피 동작 |
|----|:-:|:-:|----|----|
| G3 | 3 | 10 (격상 ack 시) | 합 ≤ machine RAM × 40% | 초과 시 sequential fallback (1 universe 씩) + `fallback_reason` |
| G4 | 4 | 12 (격상 ack 시) | 합 ≤ machine RAM × 50% | 초과 시 폭 축소 + lessons 기록 + `fallback_reason` |
| G5 | 6 | 16 (격상 ack 시) | 합 ≤ machine RAM × 60% | 초과 시 폭 축소 + checkpoint + `fallback_reason` |

budget-aware-fallback 의 fallback_reason frontmatter 의무는 존치.
