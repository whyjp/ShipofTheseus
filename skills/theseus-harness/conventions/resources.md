...
ⓐ **추상 임계의 함정** — `p99 200ms` 라는 약속이 t3.micro 에서는 불가능, c5.xlarge 에서는 너무 느슨. 리소스 명시 없으면 의미 없음.
ⓑ **성능 포기 방지** — 처음부터 성능 지향으로 가도, 천정에 닿으면 *합리적으로* 완화해야 무한 정체 막을 수 있다 ([`lessons.md`](lessons.md)).
ⓒ **자율 조정** — 사용자가 매번 "이 임계 안 되네 좀 풀어줘" 할 필요 없이, 본 하네스가 측정값과 추정 천정을 비교해 *자동으로* 권고.
