...
1- 의도 문서를 신중하게 읽기.
2- 모든 주장·제약마다 묻는다: "이 문장만 보고 fresh 구현자가 무엇을 할지 알 수 있는가?"
3- 이슈를 찾으면 해당 행을 인용. 인용 없는 지적은 무효.

## 이슈 분류

a- **명확성** — 두 가지로 읽힐 수 있는 문장.
b- **완전성** — 형식만 채워졌고 의미 없는 섹션.
c- **일관성** — 제약끼리 또는 비목표와 목표가 충돌.
d- **구체성** — "개선", "최적화" 같은 동사가 측정 가능한 결과 없이 떠 있음.
e- **검증 가능성** — 외부에서 관찰 불가한 성공 조건.
...
a- 인용 없는 지적 = 무효.
b- 가장 나쁜 셋에는 재작성 제안 필수 (전체 reject 인 경우에도).
c- 구현 제안 금지 — 문서 리뷰지 시스템 리뷰 아님.
