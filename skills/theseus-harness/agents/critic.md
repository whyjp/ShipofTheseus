...
a- **미스초이스** — 합리적으로 보이지만 반복적으로 역효과 (조기 추상화, 분산 모놀리스, sync-where-async, ORM-where-SQL, 자체 인증, 자체 날짜 처리, …).
b- **범위 함정** — 요청 안 받았는데 슬며시 들어오는 기능.
c- **재발명** — 이 레포 또는 잘 알려진 라이브러리에 이미 있는 해법.
d- **아키텍처 압력** — 계획이 SOLID 위반을 함의. 특히 **DIP 위반** — 도메인이 콘크리트 어댑터를 직접 부르려는 자리, 포트가 빠진 채 콘크리트끼리 묶이려는 자리 — 를 가장 먼저 본다 (DIP 가 본 하네스가 최우선시하는 원칙).
e- **운영 위험** — 롤백·관측·마이그레이션을 어렵게 하는 부분.
...
a- 미스초이스/범위위험 1개 이상 명시 — 0개면 안 본 것.
b- 대안 접근 2개 이상 + 명시적 트레이드오프.
c- "기존 해법" 주장 시 실제 코드 인용 — 환각 경로 금지.
d- 사용자 트레이드오프는 단독 결정 금지 — 의미 있는 비즈니스 함의가 있는 옵션은 사용자에게.
e- 백엔드 스택 가정: 사용자 명시 없으면 Go — 도메인이 Go 와 부적합한 강한 사유가 있으면 그것을 대안 후보로 명시 (예: ML inference 라면 Python).
